...
class SpotsListViewController: UIViewController {
    ...
    func sortBasedOnSegmentPressed() {
        ...
        case 2: // Avg Rating
            print("Todo")
        default:
            print("hey you shouldn't be here! our segment control just has three segments")
        }
...
extension SpotsListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        // this is a custom cell so we need to make sure its of the right subclass
...
extension SpotsListViewController: FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        if let user = user {
            tableView.isHidden = false
            print("*** We signed in with the user \(user.email ?? "unknown user")")
        }
    }
